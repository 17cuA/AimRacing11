...
namespace AIM
{
	public class CameraControl : MonoBehaviour
	{
		...
		private void FirstPersonView()
		{
			...
			float FoVAttenRate = 0.5f;//FoV減衰率
			float movedFoV = 75.0f; //プレイヤーが移動しているときのFoV
			...
			//FoV処理
			//bool moved = transform.position != prevTargetPos;
			prevTargetPos = TargetObject.position;

			//float fov = moved ?  movedFoV : stopedFoV;
			float fov = stopedFoV;
			if(carController.speed >10 && carController.speed <= 100)
			{
				fov = movedFoV;
				//fov = carController.speed / 100 * fov;
			}
		...
		private void SmoothFollowView()
		{
			...
			float attenRate = 15.0f;//減衰率

			float FoVAttenRate = 2.0f;//FoV減衰率
			float movedFoV = 60.0f; //プレイヤーが移動しているときのFoV
			float stopedFoV = 50.0f;//プレイヤーが止まっているときのFoV
