...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    public class HDRenderPipeline : RenderPipeline  //RenderPipelineレンダリング部分を記述する 
	{
        ...
        public override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            // TODO: Render only visible probes 可視プローブだけをレンダリングする
            ...
            foreach (var camera in cameras)
            {
                ...
                // Don't render reflection in Preview, it prevent them to display   //プレビューでは反射を表示しない
                if (camera.cameraType != CameraType.Reflection && camera.cameraType != CameraType.Preview
                    // Planar probes rendering is not currently supported for orthographic camera//平面のReflectionProbeは正射投影に対応していない
                    // Avoid rendering to prevent error log spamming     //エラーログのスパムを防ぐためにレンダリングを避けます
                    && !camera.orthographic)
                    // TODO: Render only visible probes //可視プローブだけレンダリング
